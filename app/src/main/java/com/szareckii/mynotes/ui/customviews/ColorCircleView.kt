...
class ColorCircleView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    companion object {
        ...
        @Dimension(unit = DP) private const val defStrokeWidthUp = 1
    }
    ...
    @Dimension(unit = PX) var strokeWidth: Float = dip(defStrokeWidthUp).toFloat()
    set(value) {
        field = value
    ...
    init {
        ...
        val defStrokeWidthPx = dip(defStrokeWidthUp).toFloat()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val weight = (radius * 2 + paddingStart + paddingEnd).toInt()
